...
function BuildFormFields2($amount) {
  //For Allocation
  pro = $amount;
  if ($amount < 0) alert("Invalid Inputs");
  console.log(res);
...
function BuildFormFields3($amount) {
  //For Allocation
  if ($amount < 0) alert("Invalid Inputs");
  console.log(res);
...
var y;
function banker() {
  if (button_run) return;
  button_run = true;
  const html_button_run = document.getElementById("btn-run");
  html_button_run.classList.add("disabled");
  ...
  console.log(alloc);
  var avail = [];
  for (var i = 0; i < res; i++) avail[i] = resource[i];
  for (var i = 0; i < pro; i++) {
    for (var j = 0; j < res; j++) {
        ...
        if (flag) {
          ...
          li.appendChild(
            document.createTextNode(
              "Process " + i + " doesn't have enough resources"
            )
          );
          ul2.append(li);
          ul1.append(ul2);
          ...
          var li = document.createElement("li");
          li.appendChild(
            document.createTextNode("Process " + i + " has enough resources")
          );
          ul2.append(li);
          ul1.append(ul2);
          prints.appendChild(ul1);
